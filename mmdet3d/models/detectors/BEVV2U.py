...
class BEVDet4D(BEVDet):
    ...
    def prepare_bev_feat(self, img, rot, tran, intrin, post_rot, post_tran,
        ...
        bda_split = torch.split(bda, split_size_or_sections=1, dim=1)
        bev_feat_split = []
        depth_split = []
        for i in range(N):
            bev_feat_split[i], depth_split[i] = self.img_view_transformer(
                [x_split[i], rot_split[i], tran_split[i], intrin_split[i], post_rot_split[i], post_tran_split[i], bda_split[i], mlp_input])
            if self.pre_process:
                bev_feat_split[i] = self.pre_process_net(bev_feat_split[i])[0]
